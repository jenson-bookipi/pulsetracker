...
export const useBlockedTickets = (clickupData, slackWebhook) => {
  const blockedTicketsWithSuggestions = useMemo(() => {
    ...
    return blockedTasks.map(task => {
      ...
      const lastUpdate = task.date_updated ? new Date(parseInt(task.date_updated)) : null
      const daysSinceUpdate = lastUpdate ? Math.floor((new Date() - lastUpdate) / (1000 * 60 * 60 * 24)) : 0
      ...
      return {
        ...
        suggestions,
        daysSinceUpdate,
        priorityLevel: task.hoursBlocked > 48 ? 'critical' : task.hoursBlocked > 24 ? 'high' : 'medium'
      }
    }).sort((a, b) => {
  ...
  const executeAction = async (task, suggestion) => {
    switch (suggestion.type) {
      ...
      case 'communication':
        if (slackWebhook) {
          return await slackWebhook.sendFollowUpMessage(task, task.assigneeName)
        }
        break
      
      case 'review':
        if (slackWebhook) {
          const message = `🔍 Review needed for: *${task.name}*\n` +
  ...
  const getBlockerStats = () => {
    const stats = {
      ...
      critical: blockedTicketsWithSuggestions.filter(t => t.priorityLevel === 'critical').length,
      high: blockedTicketsWithSuggestions.filter(t => t.priorityLevel === 'high').length,
      needsFollowUp: blockedTicketsWithSuggestions.filter(t => t.needsFollowUp).length,
      avgBlockedHours: blockedTicketsWithSuggestions.length > 0 
        ? Math.round(blockedTicketsWithSuggestions.reduce((sum, t) => sum + t.hoursBlocked, 0) / blockedTicketsWithSuggestions.length)
        : 0
    }
    return stats
  }
  ...
  const getTeamBlockerSummary = () => {
    ...
    return Object.entries(summary).map(([assignee, data]) => ({
      assignee,
      ...data,
